function listarCasos() {
    let todosOsCasos = []
    for (let contador = 1; contador <= 99; contador++){
        let contadorAoQuadrado = contador ** 2
        let stringContadorQuadrado = contadorAoQuadrado.toString()
        if (stringContadorQuadrado.length === 4){
            if (parseInt(stringContadorQuadrado.substring(0,2)) + parseInt(stringContadorQuadrado.substring(2,4)) === contador){
                todosOsCasos.push(contador)
            }
        }
    }
    return todosOsCasos
